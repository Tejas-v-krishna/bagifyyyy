...
function maxPriceFor(bundles: Bundle[]) {
...
export default function BundlesPageClient({ initialBundles }: { initialBundles: Bundle[] }) {
  ...
  // Filters state
  const [selectedMaxPrice, setSelectedMaxPrice] = useState<number>(() => maxPriceFor(initialBundles));
  const [bundleType, setBundleType] = useState<string>("");
  const [sortBy, setSortBy] = useState<string>("Newest");
  const [isFilterOpen, setIsFilterOpen] = useState(false);

  // Server-rendered bundles make this a no-op on first paint; the fetch only
  // runs when the server had nothing (degraded DB), so the page can recover.
  useEffect(() => {
    ...
    fetch("/api/bundles")
      ...
      .then((d) => {
        ...
        if (loadedBundles.length > 0) {
          setSelectedMaxPrice(maxPriceFor(loadedBundles));
        }
      })
      .catch(() => {})
