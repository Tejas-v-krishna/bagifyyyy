  ...
  try {
    ...
    try {
      await resend.emails.send({
        ...
        subject: `[Support] New ticket from ${name}${orderNumber ? ` — Order #${orderNumber}` : ''}`,
        html: `
          <div style="font-family: monospace; max-width: 600px; margin: 0 auto; padding: 24px; background: #f8f5e9;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-top: 16px;">
              <tr><td style="padding: 8px 0; color: #666; font-size: 12px; text-transform: uppercase; letter-spacing: 0.08em; width: 120px;">Ticket ID</td><td style="padding: 8px 0; font-weight: bold; color: #232D3B;">#${ticket.id.slice(0, 8).toUpperCase()}</td></tr>
              <tr><td style="padding: 8px 0; color: #666; font-size: 12px; text-transform: uppercase; letter-spacing: 0.08em;">Name</td><td style="padding: 8px 0; font-weight: bold; color: #232D3B;">${name}</td></tr>
              <tr><td style="padding: 8px 0; color: #666; font-size: 12px; text-transform: uppercase; letter-spacing: 0.08em;">Email</td><td style="padding: 8px 0;"><a href="mailto:${email}" style="color: #232D3B;">${email}</a></td></tr>
              ${orderNumber ? `<tr><td style="padding: 8px 0; color: #666; font-size: 12px; text-transform: uppercase; letter-spacing: 0.08em;">Order #</td><td style="padding: 8px 0; font-weight: bold; color: #232D3B;">${orderNumber}</td></tr>` : ''}
            </table>
            <div style="margin-top: 20px; padding: 16px; background: white; border-left: 4px solid #232D3B;">
              ...
              <p style="color: #232D3B; line-height: 1.6; margin: 0;">${message.replace(/\n/g, '<br />')}</p>
            </div>
